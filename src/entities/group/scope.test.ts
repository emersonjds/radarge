...
import { groupsForTeacher, visibleGroups } from "./scope";
...
describe("groupsForTeacher", () => {
  it("returns only the groups the teacher runs", () => {
    expect(groupIds(groupsForTeacher(groups, "t1"))).toEqual(["g1", "g3"]);
  });

  it("returns an empty list for a null teacher", () => {
    expect(groupsForTeacher(groups, null)).toEqual([]);
  });
});

describe("visibleGroups", () => {
  it("limits a teacher to the groups they run", () => {
    expect(groupIds(visibleGroups(groups, "teacher", "t2"))).toEqual(["g2"]);
  });

  it("gives coordinators and admins every group", () => {
