...
- **Stack:** Next.js 16 (App Router) + React 19, TypeScript, Tailwind CSS 4, npm. SPA com static export, sem servidor próprio; backend é a radarge-api (Node, Fastify, Postgres, Drizzle), consumida por um cliente HTTP tipado em `src/shared/lib/api/`. Slices de feature em `src/features/*`.
...
- **Sempre rode testes + build** antes de dar como pronto (`npm run test:run`, `npm run build`).
